...
% H0: known portion of the aberration function ( can be defined)
H0 = opts.H0;
F = opts.F;
Ft = opts.Ft;
...
plotReconstructedObjectandPupil(O, P, opts);
% 
% if opts.display
...
% used for LED position correction
...
while abs(err1-err2)>opts.tol&&iter<opts.maxIter
    ...
    for m = 1:Nimg
        % initilize psi for corresponding image, ROI determined by cen
        Psi0 = zeros(Np(1),Np(2),r0);
        Psi_scale = zeros(Np(1),Np(2),r0);
        cen = zeros(2,r0);
        scale0 = zeros(r0,1);
        % Construct auxiliary functions for the set of LEDs
        for p = 1:r0
            cen(:,p) = cen0-row(Ns(p,m,:));
            scale0(p) = scale(p,m);
            % initilise psi as FT(SeedImage) * P (aperature) * H0 (aberration function) 
            % downsamp clips 400x400 back to 200 x 200
            % H0: known portion of the aberration function ( can be defined)
        ...
        psi0 = Ft(Psi_scale);
        % sumphi0 = estimated intensity w/o correction term ( used for in multi LED )
        sumphi0 = sum(abs(psi0).^2,3);
        
        % Impose intensity constraint and update auxiliary function:
        if r0 == 1
            Psi = F(sqrt(I_mea).*psi0./(abs(psi0)+eps));
        else
            Psi = Proj_Fourier_v2(psi0, I_mea, sumphi0, scale0, F);
        end
        % projection 2 Update object and pupil estimates:
        dPsi = Psi-Psi0;
        Omax = abs(O(cen0(1),cen0(2)));
        
        [O,P] = P2(O,P,dPsi./repmat(H0,[1,1,r0]),Omax,cen);
        
        plotReconstructedObject(O, iter, m, cen, opts);
        ...
        err2 = err2+sqrt(sum(sum((I_mea-sumphi0).^2)));
...
function plotReconstructedObject(O, iter, m, cen, opts)
    ...
    if strcmp(opts.display,'iter')
        ...
        subplot(121); imagesc(abs(Ft(O))); axis image; colormap gray;
        title(['ampl ', txt]);

        subplot(122); imagesc(angle(O)); axis image; colormap gray; 
...
function plotReconstructedObjectandPupil(O, P, opts)
     Ft = opts.Ft;
     if strcmp(opts.display,'full') || strcmp(opts.display,'iter')
