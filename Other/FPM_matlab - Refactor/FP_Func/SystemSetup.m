...
du = lambda * f_stop/2;  % sample interval
disp(['sample interval must be < ',num2str(du)]);

disp(['NA is ',num2str(NA)]);
disp(['f# is ',num2str(f_stop)]);

%% incoherent resolution
dmin = 0.61*lambda/NA;
disp(['Incoherent resolution is ',num2str(dmin)]);
...
dx0 = 1/um_m/2;
disp(['System resolution is ',num2str(dx0)]);
...
switch imageSet
    ...
    case 2    
    dpix_c = 6; %6um pixel size on the sensor plane UI-1221LE-M-GL
    case 3    
    dpix_c = 1.67; %1.67um pixel size on the sensor plane MT9J003
    case 4
    dpix_c = 0.2 * mag; %0.2um pixel size at object 
end
...
N_obj = round(2*um_p/du)*2;  % = maxium spatial frequency achievable / sampling size at Fourier plane 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
