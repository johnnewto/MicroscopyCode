% sqr_beam propagation example, Fraunhofer pattern
%
L1=0.5;               %side length
...
w=0.011;              %source half width (m) w=0.051;              %source half width (m)
...
title('z= 0 m');
[u2,L2]=propFF(u1,L1,lambda,z); %u2=propTF(u1,L1,lambda,z); %propagation
dx2=L2/M;
...
imagesc(x2,y2,nthroot(I2,3));%stretch image contrast  imagesc(x2,y2,I2);
axis square; axis xy;
colormap('gray'); xlabel('x (m)'); ylabel('y (m)');
title(['z= ',num2str(z),' m']);
%
